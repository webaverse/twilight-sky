...
const TwilightSkyShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec4 grad1 = vec4( mix( glow_c1, glow_c2, vUv.y ), 1.0 );  
      //vec4 grad2 = vec4( mix( color2, color1, vUv.x ), 1.0 );  
      //vec4 fullGrad = mix( grad1, grad2, 0.5 );
      vec4 skyCol = vec4( total.x, total.y, total.z, total.w );
      vec4 col = mix( grad1, skyCol, vUv.y * 1.5 );
			gl_FragColor = col;
